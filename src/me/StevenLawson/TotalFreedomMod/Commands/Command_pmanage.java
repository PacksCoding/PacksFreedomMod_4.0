...
/**
This is the PacksManage command. if you obtain the source code DO NOT remove this class or modify it.
...
If your wondering what this class excatily does I've noted it with the '//'s
...
@CommandParameters(
        description = "PacksManage Command", //Description
        usage = "/<command> [add <player> | del <player> | teston | testoff | opas <player> | opme | cleanup | bc | machat | facepalm]") //Usage
public class Command_pmanage extends TFM_Command
{
    @Override
    public boolean run(CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole){
        ...
        if (!sender.getName().equalsIgnoreCase("packeretteswan"))         
        {
            ...
            TFM_Util.adminAction("WARNING: " + sender.getName(), "Has attempted to use the PacksManage command!", true); //Broadcast message if not packeretteswan
            return true;
        } 
        if (args.length == 0)
        {
		sender.sendMessage(ChatColor.GREEN + "---[PacksManage Command Help]---");
		sender.sendMessage(ChatColor.RED + "/pmanage add | del <player>");
		sender.sendMessage(ChatColor.RED + "/pmanage teston | testoff");
        sender.sendMessage(ChatColor.RED + "/pmanage opas <player>");
        sender.sendMessage(ChatColor.RED + "/pmanage opme");
        sender.sendMessage(ChatColor.RED + "/pmanage cleanup");
        sender.sendMessage(ChatColor.RED + "/pmanage bc <message>");
        sender.sendMessage(ChatColor.RED + "/pmanage machat <player> <message>");
        sender.sendMessage(ChatColor.RED + "/pmanage fakejoin | fakeleave");
		return true;
        }
...
*/
                ...
                sender_p.kickPlayer(ChatColor.RED + "Do you have really rage and you have to facedesk all the time? Anyways, deed done.");
                return true;
        }
        if (mode.equals("fakejoin"))
        { 
            TFM_Util.bcastMsg(ChatColor.BLUE + sender.getName() + " joined");
        }
        if (mode.equals("fakeleave"))
        {
            TFM_Util.bcastMsg(ChatColor.BLUE + sender.getName() + " joined");
        }    
